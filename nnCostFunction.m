...
delta1 = delta1 + (er2'*a1);
delta2 = delta2 + (er3'*a2);
%K =   (- (c==y(i,1)) ).*(log(a3))  -  ( 1.-(c==y(i,1)) ).*(log  (1.-a3) );

K = (  (- (y(i,:)) ).*(log(a3))  -  ( 1.-(y(i,:)) ).*(log  (1.-a3) ) );
...
J = (1/m)*J  +  (lambda/(2*m))*( sum(sum(Theta1(:,2:size(Theta1,2)).^2')) + sum(sum(Theta2(:,2:size(Theta2,2)).^2')));
